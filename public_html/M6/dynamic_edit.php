...
if (isset($_POST["submit"])) {
    ...
    $query = "UPDATE $table SET "; //TODO change table name
    $total = count($columns);
...
$query = "SHOW COLUMNS from $table"; //TODO change table name
$stmt = $db->prepare($query);
$columns = [];
try {
    $stmt->execute();
...
function showField($col){
    ...
    foreach($columns as $c){
        if($c["Field"] == $col){
            if(!isset($c["Default"]) && $c["Extra"] != "auto_increment"){
                return true;
            }
        }
    }
...
$query = "SELECT * from $table where id = :id"; //TODO change table name and selected columns to edit (include id if using post value)
