...
if(isset($_POST["survey"])){
    ...
    $test = "";
    //validate (extra loop because I'm lazy)
    $eachQuestionHasTwo = true;
    foreach ($survey["questions"] as $q) {
        if(count($q["answers"]) < 2){
            $eachQuestionHasTwo = false;
        }
    }
    if($eachQuestionHasTwo){
        foreach($survey["questions"] as $q){
            $test .= $q["question"];
        ...
        echo "Each question must have at least 2 answers. Please check again";
    }
    echo $test;
