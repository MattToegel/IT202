...
if (isset($_POST["submit"])) {
    ...
    if ($r) {
        changePoints(get_user_id(), 1, "Completed survey: " . $survey_id);
        flash("Response recorded successfully! Thank you!");
        die(header("Location: search_surveys.php"));
...
if ($survey_id > -1) {
    $db = getDB();
    $query = "SELECT count(1) as `attempts` from tfp_responses WHERE DATE(created) = DATE(curdate()) and user_id = :uid and survey_id = :sid";
    $stmt = $db->prepare($query);
    $r = $stmt->execute([":uid" => get_user_id(), ":sid" => $survey_id]);
    if ($r) {
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($result) {
            $attempts = safe_get($result, "attempts", 0);
            if ($attempts > 0) {
                flash("You already took this survey today");
                die(header("Location: search_surveys.php"));
            }
